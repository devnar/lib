function n1CodeColor() {
    ...
    for (j = 0; j < k; j++) {
      ...
      l = x.length;
      for (i = 0; i < l; i++) {
        x[i].innerHTML = n1CodeColorize(x[i].innerHTML, modes[j]);
      }
    }
  }
  function n1CodeColorize(x, lang) {
  ...
  "use strict";
  var n1 = {};
  n1.getParameterByName = function getParameterByName(name, url) {
      if (!url) {
  ...
  n1.hide = function (sel) {
    n1.hideElements(n1.getElements(sel));
  };
  n1.hideElements = function (elements) {
    var i, l = elements.length;
    for (i = 0; i < l; i++) {
      n1.hideElement(elements[i]);
    }
  };
  n1.hideElement = function (element) {
    n1.styleElement(element, "display", "none");
  };
  n1.show = function (sel, a) {
    var elements = n1.getElements(sel);
    if (a) {n1.hideElements(elements);}
    n1.showElements(elements);
  };
  n1.showElements = function (elements) {
    var i, l = elements.length;
    for (i = 0; i < l; i++) {
      n1.showElement(elements[i]);
    }
  };
  n1.showElement = function (element) {
    n1.styleElement(element, "display", "block");
  };
  n1.addStyle = function (sel, prop, val) {
    n1.styleElements(n1.getElements(sel), prop, val);
  };
  n1.styleElements = function (elements, prop, val) {
    var i, l = elements.length;
    for (i = 0; i < l; i++) {    
      n1.styleElement(elements[i], prop, val);
    }
  };
  n1.styleElement = function (element, prop, val) {
    element.style.setProperty(prop, val);
  };
  n1.toggleShow = function (sel) {
    var i, x = n1.getElements(sel), l = x.length;
    for (i = 0; i < l; i++) {    
      if (x[i].style.display == "none") {
        n1.styleElement(x[i], "display", "block");
      } else {
        n1.styleElement(x[i], "display", "none");
      }
    }
  };
  n1.addClass = function (sel, name) {
    n1.addClassElements(n1.getElements(sel), name);
  };
  n1.addClassElements = function (elements, name) {
    var i, l = elements.length;
    for (i = 0; i < l; i++) {
      n1.addClassElement(elements[i], name);
    }
  };
  n1.addClassElement = function (element, name) {
    var i, arr1, arr2;
  ...
  n1.removeClass = function (sel, name) {
    n1.removeClassElements(n1.getElements(sel), name);
  };
  n1.removeClassElements = function (elements, name) {
    var i, l = elements.length, arr1, arr2, j;
    for (i = 0; i < l; i++) {
      n1.removeClassElement(elements[i], name);
    }
  };
  n1.removeClassElement = function (element, name) {
    var i, arr1, arr2;
    arr1 = element.className.split(" ");
    arr2 = name.split(" ");
    for (i = 0; i < arr2.length; i++) {
  ...
  n1.toggleClass = function (sel, c1, c2) {
    n1.toggleClassElements(n1.getElements(sel), c1, c2);
  };
  n1.toggleClassElements = function (elements, c1, c2) {
    var i, l = elements.length;
    for (i = 0; i < l; i++) {    
      n1.toggleClassElement(elements[i], c1, c2);
    }
  };
  n1.toggleClassElement = function (element, c1, c2) {
    ...
    if (t2Arr.length == 0) {
      ...
      if (allPresent) {
        n1.removeClassElement(element, t1);
      } else {
        n1.addClassElement(element, t1);
      }
    } else {
      allPresent = true;
      for (j = 0; j < t1Arr.length; j++) {
      ...
      if (allPresent) {
        n1.removeClassElement(element, t1);
        n1.addClassElement(element, t2);          
      } else {
        n1.removeClassElement(element, t2);        
        n1.addClassElement(element, t1);
      }
    }
  };
  n1.getElements = function (id) {
    if (typeof id == "object") {
      return [id];
    } else {
  ...
  n1.filterHTML = function(id, sel, filter) {
  ...
  n1.sortHTML = function(id, sel, sortvalue) {
  ...
  n1.slideshow = function (sel, ms, func) {
    var i, ss, x = n1.getElements(sel), l = x.length;
    ss = {};
    ss.current = 1;
    ss.x = x;
    ...
    ss.display = function (n) {
      n1.styleElements(ss.x, "display", "none");
      n1.styleElement(ss.x[n - 1], "display", "block");
    }
    ss.start();
    return ss;
  };
  n1.includeHTML = function(cb) {
    var z, i, elmnt, file, xhttp;
    z = document.getElementsByTagName("*");
    for (i = 0; i < z.length; i++) {
      elmnt = z[i];
      file = elmnt.getAttribute("n1-include-html");
      if (file) {
        ...
        xhttp.onreadystatechange = function() {
          if (this.readyState == 4) {
            ...
            elmnt.removeAttribute("n1-include-html");
            n1.includeHTML(cb);
          }
        }      
  ...
  n1.getHttpData = function (file, func) {
    n1.http(file, function () {
      if (this.readyState == 4 && this.status == 200) {
        func(this.responseText);
      }
    });
  };
  n1.getHttpObject = function (file, func) {
    n1.http(file, function () {
      if (this.readyState == 4 && this.status == 200) {
  ...
  n1.displayHttp = function (id, file) {
    n1.http(file, function () {
      if (this.readyState == 4 && this.status == 200) {
        n1.displayObject(id, JSON.parse(this.responseText));
      }
    });
  };
  n1.http = function (target, readyfunc, xml, method) {
    var httpObj;
  ...
  n1.getElementsByAttribute = function (x, att) {
  ...
  n1.dataObject = {},
  n1.displayObject = function (id, data) {
    var htmlObj, htmlTemplate, html, arr = [], a, l, rowClone, x, j, i, ii, cc, repeat, repeatObj, repeatX = "";
    htmlObj = document.getElementById(id);
    htmlTemplate = init_template(id, htmlObj);
    html = htmlTemplate.cloneNode(true);
    arr = n1.getElementsByAttribute(html, "n1-repeat");
    l = arr.length;
    for (j = (l - 1); j >= 0; j -= 1) {
      cc = arr[j].getAttribute("n1-repeat").split(" ");
      if (cc.length == 1) {
        repeat = cc[0];
      } else {
        repeatX = cc[0];
      ...
      arr[j].removeAttribute("n1-repeat");
      ...
      if (repeatObj && typeof repeatObj == "object" && repeatObj.length != "undefined") {
        ...
        for (x in repeatObj) {
          i += 1;
          rowClone = arr[j];
          rowClone = n1_replace_curly(rowClone, "element", repeatX, repeatObj[x]);
          a = rowClone.attributes;
          for (ii = 0; ii < a.length; ii += 1) {
            a[ii].value = n1_replace_curly(a[ii], "attribute", repeatX, repeatObj[x]).value;
          }
          (i === repeatObj.length) ? arr[j].parentNode.replaceChild(rowClone, arr[j]) : arr[j].parentNode.insertBefore(rowClone, arr[j]);
        }
      } else {
        console.log("n1-repeat must be an array. " + repeat + " is not an array.");
        continue;
      }
    }
    html = n1_replace_curly(html, "element");
    htmlObj.parentNode.replaceChild(html, htmlObj);
    function init_template(id, obj) {
      var template;
      template = obj.cloneNode(true);
      if (n1.dataObject.hasOwnProperty(id)) {return n1.dataObject[id];}
      n1.dataObject[id] = template;
      return template;
    }
    function n1_replace_curly(elmnt, typ, repeatX, x) {
      ...
      while (pos1 > -1) {
        ...
        if (value != undefined) {
          ...
          if (typ == "attribute") {
            ...
            n1_replace_html(rowClone, r, value);
          }
        }
        pos1 = pos1 + 1;
      }
    ...
    function n1_replace_html(a, r, result) {
      var b, l, i, a, x, j;
      if (a.hasAttributes()) {
        b = a.attributes;
        l = b.length;
